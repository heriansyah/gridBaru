...
\begin{itemize}
  ...
  \item We develop prediction for trajectory of future position of vehicle dan bukan hanya di perempatan saja karena proaktif handover memerlukan prediksi trajectory beberapa ratus meter ke depan untuk menghindari keputusan yang salah dalam memilih gNodeB candidat yang terpat untuk menjadi serving cell.
\end{itemize}
...
\begin{equation}
  g_x = \left\lfloor x/G \right\rfloor + 1, \quad g_y = \left\lfloor y/G \right\rfloor + 1.
\end{equation}
Each cell is addressed by a unique integer identifier:
\begin{equation}
  \mathrm{gid} = (g_y - 1) \times 2000 + (g_x - 1).
\end{equation}
For example, a position $(x, y) = (42, 18)$\,m maps to $g_x = 5$, $g_y = 2$, and $\mathrm{gid} = 2004$.

\subsection{Lane Classification}
The instantaneous compass heading $\theta$ (degrees, $0^\circ$ = north, $90^\circ$ = east) is computed from recent cell-to-cell displacements.
...
\begin{figure}[!t]
\centerline{\includegraphics[width=0.95\columnwidth]{img/gridandLane.pdf}}
...
During training, for each lane $\ell$ and cell $\mathrm{gid}$, the system counts the frequency of each maneuver outcome $k \in \{L, S, R\}$:
\begin{equation}
  n_{\ell,k}(\mathrm{gid}) = \sum_{i=1}^{N_\ell} \mathbf{1}\bigl\{ \mathrm{maneuver}_i = k \;\wedge\; \mathrm{gid}_i = \mathrm{gid} \bigr\},
\end{equation}
where $N_\ell$ is the number of training trajectories on lane $\ell$ and $\mathbf{1}\{\cdot\}$ is the indicator function.
The normalized base probability vector for lane $\ell$ and cell $\mathrm{gid}$ is then:
\begin{equation}
  \mathbf{p}_{\mathrm{base}}(\mathrm{gid}, \ell) = \bigl[p_L,\; p_S,\; p_R\bigr], \quad p_k = \frac{n_{\ell,k}}{\sum_{j \in \{L,S,R\}} n_{\ell,j}}.
\end{equation}

The database stores probabilities as a 9-element direction vector $\mathbf{p}_9 = [p_1, \ldots, p_9]$, where each index maps to a compass direction: $1{=}$N, $2{=}$NE, $3{=}$E, $4{=}$SE, $5{=}$S, $6{=}$SW, $7{=}$W, $8{=}$NW, $9{=}$Stay.
The mapping from $\{p_L, p_S, p_R\}$ to the 9-direction vector depends on the lane:
\begin{equation}
  p_{d_L(\ell)} = p_L, \quad p_{d_S(\ell)} = p_S, \quad p_{d_R(\ell)} = p_R, \quad p_m = 0 \; \text{otherwise,}
\end{equation}
where the direction indices $(d_L, d_S, d_R)$ are:
Lane~1: $(7,1,3)$;
Lane~2: $(1,3,5)$;
Lane~3: $(3,5,7)$;
Lane~4: $(5,7,1)$.
...
\textbf{Deceleration ratio.}
The instantaneous speed at each time step is first computed from consecutive positions $\mathbf{p}_t = [x_t, y_t]$:
\begin{equation}
  v_t = \frac{\|\mathbf{p}_t - \mathbf{p}_{t-1}\|}{\Delta t}.
\end{equation}
The deceleration ratio $r_v$ measures the relative speed reduction near the intersection center:
\begin{equation}
  r_v = \frac{\bar{v}_{\mathrm{center}}}{\max(\bar{v}_{\mathrm{before}},\, \epsilon)},
\end{equation}
where $\bar{v}_{\mathrm{before}}$ is the mean speed from 8 to 1 steps before $\mathbf{c}_0$, $\bar{v}_{\mathrm{center}}$ is the mean speed within $\pm 2$ steps around $\mathbf{c}_0$, and $\epsilon = 10^{-6}$ avoids division by zero.
A value $r_v < 1$ indicates a slowdown, which is a reliable precursor to turning.

\textbf{Lateral drift.}
Let $\hat{\mathbf{u}}$ be the unit vector of the vehicle's incoming direction and $\hat{\mathbf{n}} = [-\hat{u}_y, \hat{u}_x]$ be its left normal.
The signed lateral projection of each trajectory point $\mathbf{p}_t$ relative to $\mathbf{c}_0$ is:
\begin{equation}
  \ell_t = (\mathbf{p}_t - \mathbf{c}_0) \cdot \hat{\mathbf{n}}.
\end{equation}
The net lateral drift is then:
\begin{equation}
  \Delta_{\mathrm{lat}} = \bar{\ell}_{\mathrm{after}} - \bar{\ell}_{\mathrm{before}},
\end{equation}
where $\bar{\ell}_{\mathrm{before}}$ and $\bar{\ell}_{\mathrm{after}}$ are the mean lateral projections before and after the crossing point, respectively.
A positive $\Delta_{\mathrm{lat}}$ indicates a drift toward the left; negative toward the right.
...
The clip operator bounds both weights to $[0, 0.6]$, preventing behavioral cues from completely overriding the database prior.
A larger $g_s$ amplifies the deceleration signal (risks false positives from traffic-induced slowdowns); a larger $g_d$ makes the left/right decision more decisive (risks noise sensitivity for small lateral displacements).

\subsection{Behavioral Bias Probability Vector}
A ternary behavioral bias $\mathbf{p}_{\mathrm{bias}} \in \mathbb{R}^3$ over $\{\mathrm{left}, \mathrm{straight}, \mathrm{right}\}$ is initialized from $w_{\mathrm{slow}}$:
\begin{equation}
  \mathbf{p}_{\mathrm{bias}}^{(0)} = \left[0.5\, w_{\mathrm{slow}},\;\; 1 - w_{\mathrm{slow}},\;\; 0.5\, w_{\mathrm{slow}}\right].
\end{equation}
The lateral drift then shifts probability mass between left and right:
\begin{align}
...
\begin{equation}
  p_i^{\mathrm{norm}} = \frac{p_i}{\sum_{j \in \{L,S,R\}} p_j}, \quad i \in \{L, S, R\}.
\end{equation}
...
\begin{equation}
  ...
  \label{eq:fusion}
\end{equation}
where $\alpha \in [0, 1]$ controls the weight of behavioral evidence; $\alpha = 0.9$ is used in all experiments.
...
Consider a northbound vehicle ($\mathrm{lane} = 1$) at $\mathrm{gid} = 2004$ with training counts 12 left, 6 straight, 2 right.
Then $\mathbf{p}_{\mathrm{base}} = [0.60, 0.30, 0.10]$.
With behavioral parameters $r_v = 0.6$, $g_s = 2.0$, $\Delta_{\mathrm{lat}} = 0.8$\,m, $d_{\mathrm{ref}} = 1.2$\,m, $g_d = 0.6$:
\begin{align}
  w_{\mathrm{slow}} &= \min((1-0.6)\cdot 2,\; 0.6) = 0.6, \\
  w_{\mathrm{drift}} &= \min((0.8/1.2)\cdot 0.6,\; 0.6) = 0.4.
\end{align}
Initial bias: $\mathbf{p}_{\mathrm{bias}}^{(0)} = [0.30, 0.40, 0.30]$.
Since $\Delta_{\mathrm{lat}} > 0$ (drifting left), after drift adjustment and normalization:
\begin{equation}
  \mathbf{p}_{\mathrm{bias}} = [0.636,\; 0.364,\; 0.000].
\end{equation}
With $\alpha = 0.9$:
\begin{multline}
  \mathbf{p}_{\mathrm{final}} = 0.1{\cdot}[0.60, 0.30, 0.10] \\
  + 0.9{\cdot}[0.636, 0.364, 0.000] = [0.632, 0.358, 0.010].
\end{multline}
The $\arg\max$ selects \textit{left}, which is correct.

Algorithm~\ref{alg:maneuver_prediction} summarizes the complete inference procedure.
